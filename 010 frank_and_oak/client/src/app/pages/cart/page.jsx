...
function CartProducts({ cartproduct, filepath }) {
  ...
  const updateQuantity = (e) => {
    ...
    const quantity = (e.target.textContent === '+') ? (cartproduct.quantity + 1) : (cartproduct.quantity - 1);

    axios.put(`http://localhost:4800/api/website/cart/update-cart/${e.target.value}`, { quantity })
      .then((response) => {
        console.log(response.data);
        const cookiedata = JSON.parse(Cookies.get('frank_user_115'));

        dispatch(fetchCart(cookiedata._id));
      })
      .catch((error) => {
        console.log(error)
      })
  ...
  const handleRemoveItem = ()=>{
    ...
    axios.delete(`http://localhost:4800/api/website/cart/delete-cart/${cartproduct._id}`)
      .then((response) => {
        console.log(response.data);
        const cookiedata = JSON.parse(Cookies.get('frank_user_115'));
      ...
      <div className='flex flex-col justify-between'>
        <div>
          <div className='flex items-center justify-between'>
            ...
            <MdClose onClick={handleRemoveItem} size={20} />
          </div>
